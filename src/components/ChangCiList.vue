    ...
    <template #reference>
      ...
      <template #default="scope">
        <el-button size="small" @click="handleCopy(scope.$index, scope.row)"
          >复制</el-button
        >
        <el-popconfirm  @confirm="handleDelete(scope.$index, scope.row)" title="Are you sure to delete this?">
    <template #reference>
...
const handleCopy = (index: number, row: User) => {
  console.log(index, row)
}

const handleMove = (index: number, row: User) => {
  console.log(index, row)
}
